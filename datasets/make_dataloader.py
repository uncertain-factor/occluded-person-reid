...
def make_dataloader(cfg):
    ...
    #创建包含训练集，测试集，展示集的数据集对象
    dataset = __factory[cfg.DATASETS.NAMES](root=cfg.DATASETS.ROOT_DIR)
    #根据训练数据集创建ImageDataset对象，传入对应的图像处理函数，数据集item为（文件路径，行人id，相机id，1，文件名），其中行人id被重置
    train_set = ImageDataset(dataset.train, train_transforms)
    train_set_normal = ImageDataset(dataset.train, val_transforms)
    num_classes = dataset.num_train_pids    #行人个数
    cam_num = dataset.num_train_cams        #相机个数
    view_num = dataset.num_train_vids       #视图个数（1）
